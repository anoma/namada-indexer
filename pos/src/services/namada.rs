...
pub async fn get_validator_set_at_epoch(
    ...
    let validator_set = rpc::get_all_validators(client, namada_epoch)
        .await
        .with_context(|| {
            format!("Failed to query Namada's consensus validators at epoch {epoch}")
        })?;
        ...
        .map(|address| async move {
            let voting_power_fut = async {
                rpc::get_validator_stake(client, namada_epoch, &address)
                    ...
                    .with_context(|| {
                        format!(
                            "Failed to query the stake of validator {address} at epoch {namada_epoch}"
                        )
                    })
            };
            ...
            let meta_and_comm_fut = async {
                let (metadata, commission) = rpc::query_metadata(client, &address, Some(namada_epoch))
                    ...
                    .with_context(|| {
                        format!(
                            "Failed to query metadata of validator {address} at epoch {namada_epoch}"
                        )
                    })?;
                metadata.zip(commission).ok_or_else(|| {
                    anyhow!(
                        "Metadata and commission must be present for validator {address} at \
                             epoch {namada_epoch}"
                    )
                })
            };
